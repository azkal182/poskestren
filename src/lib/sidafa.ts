	...
	function extractPhpSessId(cookies: any) {
	  if (cookies) {
	    const phpSessIdCookie = cookies.find((cookie:any) => cookie.includes('PHPSESSID'));
	    if (phpSessIdCookie) {
	...
	async function getBalance(rek: string): Promise<Result> {
	  await login();
	  const url = 'https://yayasan.amtsilatipusat.com/?x=YUdsemRHOXlhVjkwWVdKMWJtZGhiZz09';
	  const headers = {
	    'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9',
	    'accept-language': 'en-ID,en;q=0.9,id-ID;q=0.8,id;q=0.7,en-GB;q=0.6,en-US;q=0.5',
	    'content-type': 'application/x-www-form-urlencoded',
	...
	async function searchUser(name: string): Promise<string[]> {
	  ...
	  const headers = {
	    ...
	    'Referer': 'https://yayasan.amtsilatipusat.com/?x=YUdsemRHOXlhVjkwWVdKMWJtZGhiZz09',
	    'Referrer-Policy': 'strict-origin-when-cross-origin'
	  };
	  ...
	  try {
	    ...
	    const list: string[] = response.data.map((data: any) => parseString(data.label));
	    return list;
	  } catch (error) {
	    console.error(error);
	    return [];
